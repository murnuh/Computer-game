...
void Shapes::Boundingbox() {
	...
	max_v.x = obj_vertices[0].x;
	min_v.x = obj_vertices[0].x;

	max_v.y = obj_vertices[0].y;
	min_v.y = obj_vertices[0].y;

	max_v.z = obj_vertices[0].z;
	min_v.z = obj_vertices[0].z;

	for (unsigned int i = 1; i < linesdone; i += 1) {	// calculating max n min for every point
		var_v.x = obj_vertices[i].x;
		max_v.x = glm::max(max_v.x, var_v.x);
		min_v.x = glm::min(min_v.x, var_v.x);

		var_v.y = obj_vertices[i].y;
		max_v.y = glm::max(max_v.y, var_v.y);
		min_v.y = glm::min(min_v.y, var_v.y);

		var_v.z = obj_vertices[i].z;
		max_v.z = glm::max(max_v.z, var_v.z);
		min_v.z = glm::min(min_v.z, var_v.z);
	}

	center_v.x = (max_v.x + min_v.x) / 2;
	center_v.y = (max_v.y + min_v.y) / 2;
	center_v.z = (max_v.z + min_v.z) / 2;

	extent_v.x = (max_v.x - min_v.x) / 2;
	extent_v.y = (max_v.y - min_v.y) / 2;
	extent_v.z = (max_v.z - min_v.z) / 2;

	corner_v[0].x = center_v.x + extent_v.x;
	corner_v[0].y = center_v.y + extent_v.y;
	corner_v[0].z = center_v.z + extent_v.z;

	corner_v[1].x = center_v.x + extent_v.x;
	corner_v[1].y = center_v.y - extent_v.y;
	corner_v[1].z = center_v.z + extent_v.z;

	corner_v[2].x = center_v.x - extent_v.x;
	corner_v[2].y = center_v.y + extent_v.y;
	corner_v[2].z = center_v.z + extent_v.z;

	corner_v[3].x = center_v.x - extent_v.x;
	corner_v[3].y = center_v.y - extent_v.y;
	corner_v[3].z = center_v.z + extent_v.z;

	corner_v[4].x = center_v.x + extent_v.x;
	corner_v[4].y = center_v.y + extent_v.y;
	corner_v[4].z = center_v.z - extent_v.z;

	corner_v[5].x = center_v.x + extent_v.x;
	corner_v[5].y = center_v.y - extent_v.y;
	corner_v[5].z = center_v.z - extent_v.z;

	corner_v[6].x = center_v.x - extent_v.x;
	corner_v[6].y = center_v.y + extent_v.y;
	corner_v[6].z = center_v.z - extent_v.z;

	corner_v[7].x = center_v.x - extent_v.x;
	corner_v[7].y = center_v.y - extent_v.y;
	corner_v[7].z = center_v.z - extent_v.z;


	for (unsigned int i = 0; i < 8; i += 1) {
		vertexPositions.push_back(corner_v[i].x);
		vertexPositions.push_back(corner_v[i].y);
		vertexPositions.push_back(corner_v[i].z);
	}
}
